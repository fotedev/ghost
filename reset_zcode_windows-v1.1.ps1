# ZCode / Qoder Identity Reset v1.1
#
...
# Prerequisite: close ZCode/Qoder first (the script also force-kills them).
...
if (-not $zcodeIsAdmin) {
    $zcodeLogOut = "$env:TEMP\zcode_v1.1_result.log"
    $zcodeScript = $MyInvocation.MyCommand.Path
    $zcodeArgs = "-ExecutionPolicy Bypass -NoProfile -File `"$zcodeScript`" *> `"$zcodeLogOut`""
    ...
    if (Test-Path -LiteralPath "$env:TEMP\zcode_v1.1_done.txt") {
        Get-Content -LiteralPath "$env:TEMP\zcode_v1.1_done.txt"
    }
    exit
}
...
# === v1.1 robustness helpers (ported from reset_qoder_windows-v0.3.ps1) ========
...
$zcodeRoaming = Join-Path $env:APPDATA "ZCode"
$qoderRoaming = Join-Path $env:APPDATA "Qoder"
$zcodeUserProfile = Join-Path $env:USERPROFILE ".zcode"
...
if (-not (Test-Path -LiteralPath $zcodeRoaming)) {
    Write-Host "ZCode installation not found at: $zcodeRoaming" -ForegroundColor Red
    exit 1
}
...
Write-Host "=== ZCode / Qoder Identity Reset v1.1 ===" -ForegroundColor Cyan
Write-Host "ZCode: $zcodeRoaming" -ForegroundColor Gray
if ($qoderPresent) {
    ...
    Write-Host "Qoder: not installed (Qoder steps will SKIP)" -ForegroundColor Yellow
}
...
# ever writes while a helper still holds the files open.
...
[System.IO.File]::WriteAllText("$env:TEMP\zcode_v1.1_done.txt", ("Pass: {0} Fail: {1} Audit: {2}" -f $passCount, $failCount, $auditPath), (New-Object System.Text.UTF8Encoding $false))

if ($failCount -gt 0) {
    Write-Host "ZCode/Qoder reset v1.1 completed with failures. Review $auditPath." -ForegroundColor Red
    exit 1
}

Write-Host "ZCode/Qoder reset v1.1 completed successfully." -ForegroundColor Green
